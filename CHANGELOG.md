...
- The Duplicate button on each layer card moved back up into the header, immediately to the left of the delete (×) button; its footer now holds only Move Up/Down and the Group checkbox, which is left-aligned instead of centered.
